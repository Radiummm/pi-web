...
export function AppShell() {
  ...
  const handleAskInNewChat = useCallback(async (prompt: string, sourceSessionId?: string, sourceEntryId?: string) => {
    ...
    if (sourceSessionId && sourceEntryId) {
      try {
        const result = await sendAgentCommand<{ newSessionId?: string }>(sourceSessionId, {
          type: "fork_branch",
          entryId: sourceEntryId,
        });
        branchSessionId = result?.newSessionId;
      } catch (error) {
        console.error("Failed to fork quoted chat:", error);
      }
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, minHeight: 0 }}>
              <ChatWindow
                key={quoteChat.id}
                session={quoteChat.sessionId ? { id: quoteChat.sessionId, path: "", cwd: quoteChat.cwd, created: "", modified: "", messageCount: 0, firstMessage: "", transient: false } : null}
                newSessionCwd={quoteChat.sessionId ? null : quoteChat.cwd}
                newSessionDraftKey={quoteChat.sessionId ? null : `quote:${quoteChat.id}`}
